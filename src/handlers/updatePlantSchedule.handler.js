...
const { updatePlantSchedule } = require('../utils/plantsClient');
const util = require('util');
const locale = require('../locales/en-GB');
const constants = require('../utils/constants');
const { prepareUpdatedPlantResponse } = require('../utils/responses');

const AskUpdatePlantScheduleIntentHandler = {
  canHandle(handlerInput) {
    return (
      Alexa.getRequestType(handlerInput.requestEnvelope) === 'IntentRequest' &&
      // eslint-disable-next-line prettier/prettier
      Alexa.getIntentName(handlerInput.requestEnvelope) === 'UpdatePlantScheduleIntent'
    );
  },
  async handle(handlerInput) {
    ...
    try {
      const updatePlantResponse = await updatePlantSchedule(
        userId,
        plantName,
        newWaterSchedule
      );
      console.debug(updatePlantResponse);
    } catch (error) {
      // Logging the error
      console.debug(
        `Error updating a plant with the following data: ${util.inspect({
          userId: userId,
          plantName: plantName,
    ...
    const speechText = prepareUpdatedPlantResponse(plantName, newWaterSchedule);
...
module.exports = AskUpdatePlantScheduleIntentHandler;
